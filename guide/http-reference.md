...
## statu_code
`HTTP-STATU-CODE` 用来指定 HTTP 请求的响应状态
...
@app.route ('/')
def index ():
    app.response.statu_code = 500
    return "hi"
```

## mime-type
`mime-type` 用于提示浏览器如何处理发送过去的数据.
```python
".txt"   : "text/plain",
".htm"   : "text/html",
".html"  : "text/html",
...
### 设置 mime-type
```python
from urouter import uRouter, get_mime_type
app = uRouter(keep_alive=True)

@app.route ('/')
def index ():
    app.response.mime_type = get_mime_type(".jpg")
    return b'jpg-data'
```